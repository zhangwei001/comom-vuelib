    ...
    <Modal
        ...
        <div class="cloud-body">
            <div class="cloud-content" :class="{'max-win': maxDialog}">
                ...
                <div class="btn-bar">
                    <Button type="ghost" @click="addFolder" :disabled="!haveAuth"><i class="iconfont_daydao_common">&#xe703;</i><span class="m-l-10">新建文件夹</span></Button>
                    <Button type="ghost" class="right m-l-10" @click="show = false">取消</Button>
                    <Button type="primary" class="right" @click="saveToCloud" :disabled="!haveAuth">保存</Button>
                </div>
            </div>
        </div>
        <div slot="footer" ></div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            , getPermission(){
                let t = this;
                return Ajax.havePermission().then(data=>{
                    let isAdmin = !!data.dayAdminFlg;
                    t.corp.isAdmin = isAdmin;
                    if(isAdmin){
                        // 网盘管理员
                        t.corp.auth = true;
                    }else{
                        // 非网盘管理员
                        let org = data.OrgPermission || [];
                        let folderPrivilege = {};

                        org.forEach(item=>{
                            folderPrivilege[item.fileId] = item.privilege;
                        });

                        t.corp.folderPrivilege = folderPrivilege;
            ...
            , setOrgAuth(fileId){
                ...
                // 默认只能查看
                let privilege = t.corp.folderPrivilege[fileId] || 4;
             ...
             * @param diskType 网盘类型
             * */
            , addFolderCallback(isAdd, name, index){
                let t = this;
                let type = t.tabName;
                let diskType = t.currDiskType;

                if(isAdd){
                    let nav = t[type].folderNav;
                    let parentId = nav[nav.length -1].uuid;
                    t.loadingData = true;
                    Ajax.addFolder(name, diskType, parentId).then(data=>{
                        t.getCloudFileList(diskType, parentId).then(data=>{
            ...
            , saveToCloud(){
                ...
                let nav = t[type].folderNav;
                Ajax.saveToCloud(t.currDiskType, t.selectedFileId, nav[nav.length - 1].uuid).then(data=>{
                    if(data.result === 'true'){
            ...
            , openFolder(index){
                ...
                if(item && item.fileType === t.fileType.folder && !item.rename){
