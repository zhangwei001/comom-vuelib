        ...
        ,methods:{
            ...
            ,initTree:function(){
                ...
                seajs.use(["commonStaticDirectory/plugins/zTree/zTreeStyle/zTreeStyle.css","commonStaticDirectory/plugins/zTree/jquery.ztree.all.min"],function () {
                    ...
                    t.setValue(t.val,t.showVal,true);
                });
            }
            /**
             * 关闭下拉框
             ...
             * @param showVal Array
             * */
            ,setValue(val,showVal , isContainChild=false){
                var t = this;
                    ...
                    if(aNodes.length){
                        for(var i=0;i<aNodes.length;i++){
                            if(aVal.indexOf(aNodes[i].id) != -1 || aShowVal.indexOf(aNodes[i].name) != -1){ //选中匹配,用或的关系来处理，要保证树的每个name唯一，因为后端有时候不好同时传id和name过来
                                isHasSelected = true;
                                t.ztreeResultObj.checkNode(aNodes[i],true,isContainChild,true); //布尔值的含义：选中，不关联树，触发onCheck回调
